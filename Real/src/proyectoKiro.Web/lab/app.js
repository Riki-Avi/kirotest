/**
 * CODELAB — Clean 3D Laboratory Flask ASCII Engine
 * 
 * Features:
 * - Upright 3D Erlenmeyer Flask (Matraz de laboratorio) background
 * - Glass contours, chemical orange liquid fill, rising bubbles & vapor
 * - Responsive 3D mouse rotation (no scroll tilt or code background text)
 * - Clean editorial page layout
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function render() {
    ...
    if (now - lastTime >= 500) {
      ...
      nodeCounter.textContent = (cols * rows).toLocaleString();
      frameCount = 0;
      lastTime = now;
    }
    ...
    // Interactive 3D Rotation with gentle auto-sway (No scroll tilt)
  ...
  if (startBtn) {
    startBtn.addEventListener('click', () => {
      alert('¡Bienvenido a CODELAB! Iniciando tu primera sesión de práctica de entrevistas con la IA Tutor.');
    });
  }
  ...
  audioToggleBtn.addEventListener('click', () => {
    if (!isAudioPlaying) {
      startAudio();
      audioToggleBtn.innerHTML = '<span class="audio-icon">&#10074;&#10074;</span> DETENER SONIDO AMBIENTAL';
      audioToggleBtn.style.background = 'var(--accent-orange)';
      audioToggleBtn.style.color = '#000000';
    } else {
      stopAudio();
      audioToggleBtn.innerHTML = '<span class="audio-icon">&#9654;</span> ACTIVAR SONIDO AMBIENTAL TECH';
